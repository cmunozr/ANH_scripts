...
PrintggiNext<-function(fnm,iNxt){
  ...
  SC<-iNxt$DataInfo%>%select(-starts_with('f'))
...
PrintRefiNext<-function(fnm,catnm,iNxt){
  ...
  if(max(nchar(labsxx))>=10) {
    labsx<-gsub('\\b(\\pL{0,3})\\pL{2,}|.','\\U\\1-',labsxx,perl=TRUE)
    point.df$Site<-labsx}
...
PrintggiNextInc<-function(fnm,iNxt){
  ...
  dev.off()
  print(g3)
  write.csv(point.df,file.path(WDOut, paste(fnm,'Inc_Asym.csv',sep='')))
...
cat.c<-c("Plataf","Cobertura","habitat")#c("Plataf","Red.Hidrica","Orden")
...
catnm<-"Cobertura" #main factor for análisis
gnm<-"Rep" #group prefix
...
Data.r<-Data.r[Data.r$class=="Reptilia",]
selr<-unique(Data.r$eventID)
Data.e<-Data.e[Data.e$eventID%in%selr,]
#quality checks##
...
cov<-cov[cov$parentEventID%in%UM,]
cov<-complete_cols(cov,Data.e,"parentEventID",c("habitat"))
...
ommt<-c("") #c("Recorrido en lancha","Recorrido Libre") #method to be omitted
ompv<-c("") #c("ANH_380","ANH_64","ANH_65")
Data.r2<-Data.r%>%filter((!parentEventID%in%ompv)&(!samplingProtocol%in%ommt))
nsp<-unique(Data.r2$parentEventID)
...
text(sit.scl,rownames(sit.scl),cex=.7,pos=3)
legend(x=1.5*max(p$default[,1]),y=0.8*max(p$default[,2]),paste("grupo",c(1:length(grl))),pch=14+c(1:length(grl)),col=1+c(1:length(grl)),
       cex=0.7, y.intersp = 1,x.intersp=0.3, bty="n")
dev.off()
...
#9b) Incidence data
outf<-file.path(WDOut,paste("RDA_res_Inc",gnm,".txt",sep=""))
sink(outf)
